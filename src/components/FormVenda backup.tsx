...
export default function FormVenda({ isOpen, onClose, onSaved, venda }: FormVendaProps) {
  ...
  useEffect(() => {
    ...
    if (venda) {
      ...
      setForm({ codigo: '', cliente_id: '', vendedor_id: '', valor_total: 0, lucro_parcial: 0, lucro_final: 0, data_venda: new Date().toISOString().split('T')[0], situacao: 'Finalizada', tipo_pagamento: 'À Vista', observacao: '' });
      setItens([]);
    }
  }, [venda, isOpen]);
